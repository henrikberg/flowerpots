# Parametric Flowerpot Generator

Generates STEP files for 3D-printable flowerpots using [CadQuery](https://cadquery.readthedocs.io/).

## Install

```bash
pip3 install -r requirements.txt
```

## Generate a pot

```bash
python3 generator.py --diameter 120 --height 100 --wall 3 --base 4 pot.step
```

## Available parameters

| Parameter          | Default         | Description                                            |
|--------------------|-----------------|--------------------------------------------------------|
| `--diameter`       | 120             | Outer top diameter (mm)                                |
| `--height`         | 100             | Total height (mm)                                      |
| `--wall`           | 3               | Wall thickness (mm)                                    |
| `--base`           | 4               | Base thickness (mm)                                    |
| `--drain-diameter` | 8               | Drainage hole diameter (mm), 0 to disable              |
| `--drains`         | 4               | Number of drainage holes (1 centered, >1 in a circle)  |
| `--taper`          | 5               | Wall draft angle (degrees)                             |
| `--rim-thickness`  | 2               | Extra rim thickness (mm)                               |
| `--rim-height`     | 5               | Rim height (mm)                                        |
| `--foot`           | 3               | Height of bottom ventilation feet (mm), 0 to disable   |
| `--rings`          | 2               | Number of concentric bottom ventilation rings          |
| `filename`         | flowerpots.step | Output STEP file path                                  |

## Example batch

```bash
python3 generator.py --diameter 100 --height 80 small.step
python3 generator.py --diameter 150 --height 120 --wall 4 large.step
python3 generator.py --diameter 120 --height 100 --drains 4 --drain-diameter 6 four_holes.step
python3 generator.py --diameter 120 --height 100 --foot 6 --rings 2 ventilated.step
```
